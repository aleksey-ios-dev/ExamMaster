...
class APIClient: Service {
  func fetchSubjects(completion: (_ subjects: [Subject]?, _ error: ModelTreeError?) -> Void) -> Void {
//    let delayTime = DispatchTime.now()
//    let delayTime = DispatchTime.now(dispatch_time_t(DISPATCH_TIME_NOW), Int64(0.1 * Double(NSEC_PER_SEC)))
...
//    }
  ...
  func fetchTopics(for subject: Subject, completion: (_ topics: [Subject]?, _ error: Error?) -> Void) -> Void {
